...
export default function MediosSection({
  ...
  nfmt, dfmt,
  // transferencias
  optsTransf, transfSel, setTransfSel, transfMonto, setTransfMonto, aplicTransf, addTransf, delTransf,
  // cajas
  optsCajas, cajaSel, setCajaSel, cajaMonto, setCajaMonto, aplicCajas, addCaja, delCaja,
  // aplicaciones
  optsAp, apSel, setApSel, apMonto, setApMonto, aplicAps, addAp, delAp,
}) {
  const allChequesChecked = cheques.length > 0 && selCheques.size === cheques.length;
  const toggleAllCheques = (e) => {
    if (!cheques.length) return;
    if (e.target.checked) setSelCheques(new Set(cheques.map((_, i) => i)));
        ...
        <div className={styles.cardBody}>
          <div className={styles.filtersGrid}>
            ...
            <div className={styles.filterItem}>
              ...
              <button className={styles.btn} onClick={cargarCheques} disabled={!file}>Cargar</button>
            </div>
            <div className={styles.filterItem}>
              <label className={styles.label}>&nbsp;</label>
              <button className={styles.btn} onClick={cancelarCheques} disabled={!cheques.length}>Cancelar</button>
            </div>
          </div>
          ...
          <div className={styles.tableContainer} style={{ marginTop: 8 }}>
            <table className={styles.table}>
              ...
              <tbody>
                {!cheques?.length ? (
                  <tr><td className={styles.noResults} colSpan={6}>No hay cheques importados todavía.</td></tr>
                ) : (
                  cheques.map((c, i) => (
      ...
      <div className={styles.card}>
        ...
        <div className={styles.cardBody}>
          <div className={styles.filtersGrid}>
            ...
            <div className={styles.filterItem}>
              <label className={styles.label}>&nbsp;</label>
              <button className={styles.btn} onClick={addTransf} disabled={!transfSel || !transfMonto}>Agregar</button>
            </div>
          </div>
            ...
            <ul className={styles.listSimple}>
              {aplicTransf.map((t, i) => (
                <li key={i}>
                  ...
                  <button className={styles.smallBtn} onClick={() => delTransf(i)}>Quitar</button>
                </li>
              ))}
            </ul>
          )}
      ...
      <div className={styles.card}>
        ...
        <div className={styles.cardBody}>
          <div className={styles.filtersGrid}>
            ...
            <div className={styles.filterItem}>
              <label className={styles.label}>&nbsp;</label>
              <button className={styles.btn} onClick={addCaja} disabled={!cajaSel || !cajaMonto}>Agregar</button>
            </div>
          </div>
          ...
          {aplicCajas.length > 0 && (
            <ul className={styles.listSimple}>
              {aplicCajas.map((t, i) => (
                <li key={i}>
                  <span>{t.label}</span> <strong>$ {nfmt(t.monto)}</strong>{" "}
                  <button className={styles.smallBtn} onClick={() => delCaja(i)}>Quitar</button>
                </li>
              ))}
            </ul>
          )}
      ...
      <div className={styles.card}>
        ...
        <div className={styles.cardBody}>
          <div className={styles.filtersGrid}>
            ...
            <div className={styles.filterItem}>
              <label className={styles.label}>&nbsp;</label>
              <button className={styles.btn} onClick={addAp} disabled={!apSel || !apMonto}>Agregar</button>
            </div>
          </div>
          ...
          {aplicAps.length > 0 && (
            <ul className={styles.listSimple}>
              {aplicAps.map((t, i) => (
                <li key={i}>
                  <span>{t.label}</span> <strong>$ {nfmt(t.monto)}</strong>{" "}
                  <button className={styles.smallBtn} onClick={() => delAp(i)}>Quitar</button>
                </li>
              ))}
            </ul>
          )}
